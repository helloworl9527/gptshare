...
func New(cfg Config) (*Manager, error) {
	if cfg.DB == nil || cfg.Username == "" || len(cfg.JWTKey) < 32 || len(cfg.RateLimitKey) < 32 || len(cfg.TOTPSecret) < 20 {
	...
	return &Manager{
		...
		jwtKey:       append([]byte(nil), cfg.JWTKey...),
		limiter:      newLimiter(cfg.DB, cfg.RateLimitKey),
...
func (m *Manager) Password(ctx context.Context, username, password, clientIP, csrfToken string) (string, error) {
	...
	m.challenges[hash] = &challenge{
		...
		ipHMAC:    m.limiter.ipDigest(clientIP),
...
func (m *Manager) TOTP(ctx context.Context, rawChallenge, passcode, clientIP, csrfToken string) (*Session, error) {
	...
	csrfDigest := sha256Digest(csrfToken)
	m.mu.Lock()
...
func (m *Manager) createSession(ctx context.Context, totpStep int64, now time.Time) (*Session, error) {
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO admin_sessions(jti_hash,issued_at,expires_at)
		VALUES (?, ?, ?)`, decodedHash, stamp, expires.Format(time.RFC3339Nano)); err != nil {
		return nil, err
	}
