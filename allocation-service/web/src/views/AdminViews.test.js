...
describe('P2 admin views', () => {
  ...
  it('generates cards and keeps one-time plaintext visible only in generated panel', async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(response({ csrf_token: 'c'.repeat(43) }))
      .mockResolvedValueOnce(response({ cards: [{ id: 5, code: '2345-6789-ABCD', code_suffix: 'ABCD', duration_days: 45, status: 'unused' }] }, 201))
      .mockResolvedValueOnce(response({ cards }))
    const wrapper = await render(Cards, fetchMock)
    await wrapper.get('button.compact-action').trigger('click')
    await wrapper.find('#quantity').setValue(1)
    await wrapper.find('#duration').setValue(45)
    await wrapper.find('.modal-form').trigger('submit')
    await flushPromises()
    ...
    expect(JSON.parse(generateCall[1].body)).toMatchObject({ quantity: 1, duration_days: 45 })
    wrapper.unmount()
  })

  it('offers one-to-ninety day input, common presets, and the ninety-day extension budget', async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(response({ cards }))
      .mockResolvedValueOnce(response({ csrf_token: 'c'.repeat(43) }))
    const wrapper = await render(Cards, fetchMock)
    ...
    expect(duration.attributes()).toMatchObject({ min: '1', max: '90', step: '1' })
    expect(wrapper.text()).toContain('可输入 1–90 天任意整数')
    for (const days of [7, 14, 30, 90]) {
      expect(wrapper.findAll('button').some((button) => button.text() === `${days} 天`)).toBe(true)
    }
    await wrapper.get('[aria-label="关闭弹窗"]').trigger('click')
    await flushPromises()
    const extendButton = wrapper.findAll('button').find((button) => button.text() === '延期' && button.attributes('disabled') === undefined)
    await extendButton.trigger('click')
    expect(wrapper.find('#extend-days').attributes('max')).toBe('60')
    expect(wrapper.text()).toContain('当前最多可延期 60 天，总有效期上限为 90 天')
    wrapper.unmount()
  })
