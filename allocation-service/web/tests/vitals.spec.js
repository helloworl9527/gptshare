...
test('desktop dashboard and four-page navigation have no console errors', async ({ page }) => {
  ...
  await expect(page.getByLabel('有效期天数')).toHaveAttribute('min', '1')
  await expect(page.getByLabel('有效期天数')).toHaveAttribute('max', '90')
  await expect(page.getByText('可输入 1–90 天任意整数。')).toBeVisible()
  await page.getByRole('button', { name: '14 天' }).click()
  await expect(page.getByLabel('有效期天数')).toHaveValue('14')
  await page.getByLabel('有效期天数').fill('45')
  await expect(page.getByLabel('有效期天数')).toHaveValue('45')
